...
class Human_tracking():
    ...
    def pub_sub(self):
        sub_list=[]
        # subscriber
        rgb_sub=message_filters.Subscriber(self.camera_frame+"/camera/color/image_raw",Image)
        sub_list.append(rgb_sub)
        if dpt_align:
            dpt_sub=message_filters.Subscriber(self.camera_frame+"/camera/aligned_depth_to_color/image_raw",Image)
        else:
            dpt_sub=message_filters.Subscriber(self.camera_frame+"/camera/depth/image_rect_raw",Image)
        sub_list.append(dpt_sub)
        info_sub=message_filters.Subscriber(self.camera_frame+"/camera/depth/camera_info",CameraInfo)
    ...
    def point_translation(self,person):
        return (person[-3][2]/1000, person[-3][1]/1000, -person[-3][0]/1000)
    ...
    def broadcast_human(self,rect_list):
        for person in rect_list:
            ...
            trans=(self.point_translation(person)[0],0,self.point_translation(person)[2])
            ...
            try:
                ...
                br_human.sendTransform(trans,rot,rospy.Time.now(),"/human",self.camera_frame_debug)
